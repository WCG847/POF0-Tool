...
class YCHUNK:
        ...
        while True:
            try:
                ...
                if TMP_IND2 == TYPE3:
                    ...
                    self.YCT.seek(-2, 1)
                    REAL_W = (
                        (TMP_INDICE << 24) | (TMP_W1 << 16) | (TMP_W2 << 8) | TMP_W3
                    )
                    REAL_IND = (
                        (
                            (REAL_W << (32 + 2) & 18446744073709551615) >> (32 + 2)
                            & 18446744073709551615
                        )
                        << 2
                    ) + 8
                    print(f"converted {TMP_INDICE} to {REAL_IND}")
                    self.YCT.seek(3, 1)
    ...
    def get_file_from_user():
        ...
        if Tk is None:
            print(
                "Error: tkinter is required for GUI file selection but is not installed."
            )
            sys.exit(1)
